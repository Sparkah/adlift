...
$("#viewnav").addEventListener("click", (e) => {
  ...
  $("#view-poll").classList.toggle("hidden", v !== "poll");
  if (v === "auction") renderAuction();
  if (v === "poll" && !POLL.options.length) pollInit();
});

// ---------- poll loop (the product: sponsored poll -> live cover -> CTR -> regenerate) ----------
let POLL = { pool: [], idx: 0, cycle: 0, ctr: 0.005, options: [], busy: false };

async function pollLoadPool() {
  if (POLL.pool.length) return;
  try {
    const r = await fetch(S.static ? "data/fixtures/replay_battle_merge.json" : "/api/replay?id=battle_merge");
    const run = await r.json();
    const imgs = [];
    for (const rd of run.rounds) for (const v of rd.variants) if (v.image?.dataUrl) imgs.push({ img: v.image.dataUrl, head: v.headline });
    POLL.pool = imgs;
  } catch {}
  if (!POLL.pool.length) POLL.pool = [
    { img: assetURL("/data/baseline/battle_merge_after.png"), head: "Battle Merge" },
    { img: assetURL("/data/baseline/battle_merge_before.png"), head: "Merge to win" },
  ];
}
function pollPick(n, seedWith) {
  const opts = seedWith ? [{ ...seedWith, votes: 0, _el: null }] : [];
  while (opts.length < n) { opts.push({ ...POLL.pool[POLL.idx % POLL.pool.length], votes: 0, _el: null }); POLL.idx++; }
  return opts;
}
function pollRenderOptions() {
  const box = $("#pollOptions"); box.innerHTML = "";
  for (const o of POLL.options) {
    const c = el("div", "poption");
    c.innerHTML = `<img class="pthumb" src="${o.img}" alt=""><div class="pmeta"><div class="phead">${esc(o.head)}</div><div class="pbar"><div class="pfill"></div></div><div class="pvotes"><span class="pv">0</span><span class="pw"></span></div></div>`;
    box.appendChild(c); o._el = c;
  }
  $("#pollTotal").textContent = "";
}
function pollStep(s) { for (const li of document.querySelectorAll("#cycleSteps li")) li.classList.toggle("active", li.dataset.step === s); }

async function pollRunCycle() {
  if (POLL.busy) return; POLL.busy = true;
  pollStep("poll");
  const weights = POLL.options.map(() => 0.5 + Math.random());
  const total = 600;
  for (let t = 1; t <= total; t++) {
    const wsum = weights.reduce((a, b) => a + b, 0);
    let r = Math.random() * wsum, k = 0; while (r > weights[k] && k < weights.length - 1) { r -= weights[k]; k++; }
    POLL.options[k].votes++;
    if (t % 20 === 0) {
      const max = Math.max(...POLL.options.map((o) => o.votes));
      for (const o of POLL.options) { o._el.querySelector(".pfill").style.width = (o.votes / max * 100) + "%"; o._el.querySelector(".pv").textContent = o.votes; }
      $("#pollTotal").textContent = t + " votes";
      await sleep(16);
    }
  }
  const winner = POLL.options.reduce((a, b) => (b.votes > a.votes ? b : a));
  for (const o of POLL.options) o._el.classList.toggle("win", o === winner);
  winner._el.querySelector(".pw").textContent = "WINNER";
  await sleep(700);
  pollStep("winner"); await sleep(900);
  pollStep("ctr"); POLL.cycle++;
  const prev = POLL.ctr;
  POLL.ctr = Math.min(0.04, POLL.ctr + (POLL.cycle === 1 ? 0.022 : 0.006) + Math.random() * 0.004);
  $("#pollCtr").innerHTML = `Winner pushed to the live CrazyGames cover.<br>Real CTR: <span class="big">${(POLL.ctr * 100).toFixed(1)}%</span> <span style="color:var(--muted)">(was ${(prev * 100).toFixed(1)}%)</span>`;
  await sleep(1100);
  pollStep("regen");
  $("#pollInsight").innerHTML = `Cycle ${POLL.cycle}: <b>${esc(winner.head)}</b> won on real votes &rarr; live cover &rarr; CTR <b>${(POLL.ctr * 100).toFixed(1)}%</b>. AdLift generates 4 fresh variants from the winner...`;
  await sleep(1000);
  POLL.options = pollPick(4, winner); pollRenderOptions();
  await sleep(400);
  pollStep("loop");
  $("#pollInsight").innerHTML += ` <b>New poll ready</b> - run it again to continue the loop.`;
  POLL.busy = false;
}

async function pollInit() {
  await pollLoadPool();
  POLL.cycle = 0; POLL.ctr = 0.005; POLL.idx = 0;
  POLL.options = pollPick(4); pollRenderOptions(); pollStep("poll");
  $("#pollCtr").innerHTML = `Baseline CrazyGames CTR: <span class="big">0.5%</span>`;
  $("#pollInsight").textContent = "A sponsored poll inside an AI chat crowdsources which cover wins - real human preference, not a guess. Hit Run the cycle.";
}
$("#pollRun").onclick = () => pollRunCycle();
$("#pollReset").onclick = async () => { POLL.busy = false; POLL.cycle = 0; POLL.ctr = 0.005; POLL.idx = 0; POLL.options = []; await pollInit(); };
