...
import { initOvermind, traced } from "./overmind.js";
...
// fire-and-forget: enables Overmind supervision when OVERMIND_API_KEY is set
initOvermind(env.OVERMIND_API_KEY);
...
function activeModel() {
  return LLM_PROVIDER === "openai" ? OPENAI_MODEL : LLM_PROVIDER === "anthropic" ? ANTHROPIC_MODEL : CF_TEXT_MODEL;
}

async function llmRaw({ system, user, maxTokens, temperature }) {
...
// LLM call -> parsed JSON, traced to Overmind (when enabled) with per-step labels.
export async function llmJSON({ system, user, maxTokens = 1200, temperature = 0.8, label = "chat" }) {
  return traced(
    `llm.${label}`,
    {
      "gen_ai.system": LLM_PROVIDER === "cf" ? "cloudflare-workers-ai" : LLM_PROVIDER,
      "gen_ai.operation.name": "chat",
      "gen_ai.request.model": activeModel(),
      "gen_ai.prompt": (String(system) + "\n\n" + String(user)).slice(0, 4000),
      "adlift.step": label,
    },
    async (span) => {
      const raw = await llmRaw({ system, user, maxTokens, temperature });
      if (span) span.setAttribute("gen_ai.completion", String(raw).slice(0, 4000));
...
export async function genImage(prompt, opts = {}) {
  return traced(
    "image.gen",
    {
      "gen_ai.system": CF_ACCOUNT_ID && CF_AI_TOKEN ? "cloudflare-flux" : "pollinations",
      "gen_ai.operation.name": "image.generate",
      "gen_ai.request.model": CF_IMAGE_MODEL,
      "gen_ai.request.type": "image",
      "gen_ai.prompt": String(prompt).slice(0, 1000),
      "adlift.step": "image_gen",
    },
    async (span) => {
      const out = await _genImage(prompt, opts);
      if (span) span.setAttribute("gen_ai.response.provider", out.provider);
      return out;
    },
  );
}
async function _genImage(prompt, { width = 1024, height = 768 } = {}) {
